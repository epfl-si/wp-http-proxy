 ...
 * @constructor
 */
function Cache (opts, inject) {
    const Redis = (inject && inject.Redis) ? inject.Redis : IORedis,
          CachePolicy = (inject && inject.CachePolicy ) ? inject.CachePolicy : RealCachePolicy;
     ...
     * @return doc
     */
    this.writeBack = async function(req, res) {
        res = await res;
        const key = stringifyCacheKey(this.getCacheKey(req)),
    ...
    // There doesn't seem to be a completion callback for this afaict
    this.stop = () => this.redis.disconnect()
}  // Cache constructor
